...
const CFG_NAME = "Iqbal"; // Nama pemilik website
const CFG_TITLE = "Zansky | Portfolio"; // Judul pada tab browser
const CFG_GITHUB_USER = "Iqbaal06"; // Ganti dengan username GitHub asli Anda
...
// Theme Mode Engine (Light/Dark Toggle)
...
function updateThemeIcon(isLight) {
    ...
    if (isLight) {
        ...
        document.getElementById('theme-toggle-btn').title = "Ubah ke Mode Gelap";
    } else {
        icon.className = "fa-solid fa-sun";
        document.getElementById('theme-toggle-btn').title = "Ubah ke Mode Terang";
    }
}
...
// Execute initTheme immediately to prevent flash of dark theme
initTheme();
...
const stringsToType = ["Full Stack Developer", "UI/UX Designer", "Roblox Developer"];
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Set dynamic names from configuration
    document.title = CFG_TITLE;
    const userTitleNode = document.getElementById('user-title');
    if (userTitleNode) userTitleNode.innerText = CFG_NAME;
    const footerAuthorNode = document.getElementById('footer-author');
    if (footerAuthorNode) footerAuthorNode.innerText = CFG_NAME;
